layui.use(['jquery','layer', 'table','form','upload','element'], function() {
    ...
    var uploadInst = upload.render({
        ...
        ,data: {"path":"E:\\img"} //传递到后端的参数，表示上传路径
    ...
    <!--<img class="layui-anim" src="/img/${room.roomPic}" width="135" height="135">-->
    /*************************自定义函数************************/
    //初始化所有的客房数据
    function loadAllRooms(){
        $.post(
            ...
            layer.msg("数据请求异常1！",{icon: 7,time:2000,anim: 3,shade:0.5});
        })
    }
        ...
        $.post(
            ...
            function (data){
                if(data === 'success'){
                    //重新加载客房数据
                    loadAllRooms();
                    layer.msg("客房删除成功！",{icon: 1,time:2000,anim: 1,shade:0.5});
                }else{
                    layer.msg("客房删除失败！",{icon: 2,time:2000,anim: 2,shade:0.5});
                }
            },"text" //text : 表示后端响应的是文本
        ).error(function (){
            layer.msg("数据请求异常2！",{icon: 7,time:2000,anim: 3,shade:0.5});
        })
    }
    ...
    function updRoomsStatus(roomid,roomStatus){
        $.post(
            ...
            layer.msg("数据请求异常3！",{icon: 7,time:2000,anim: 3,shade:0.5});
        })
    }
    ...
    $("ul").eq(2).on('click','button',function (){
        ...
        if(event == 'del'){ //点击了删除
            ...
            layer.confirm('真的要把客房改为空闲么', function(index){
                //向服务端发送修改客房的状态  ： 2 -> 0
    ...
    function loadAllRoomType(){
        $.post(
            ...
            layer.msg("数据请求异常4！",{icon: 7,time:2000,anim: 3,shade:0.5});
        })
    }
    ...
    function checkRoomNum(roomNum){
        ...
        $.post(
            ...
            layer.msg("数据请求异常5！",{icon: 7,time:2000,anim: 3,shade:0.5});
        })
    }
    ...
    function saveRoom(saveJsonRoom){
        $.post(
            ...
            layer.msg("数据请求异常6！",{icon: 7,time:2000,anim: 3,shade:0.5});
        })
    }
